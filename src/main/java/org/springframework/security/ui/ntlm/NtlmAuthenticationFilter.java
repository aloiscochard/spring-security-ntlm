...
public class NtlmAuthenticationFilter extends GenericFilterBean implements InitializingBean {
    ...
    private void authenticate(final HttpServletRequest request, final HttpServletResponse response, final HttpSession session,
            final NtlmPasswordAuthentication auth) throws IOException {
    ...
    private void processType1Message(final String message, final HttpSession session, final UniAddress dcAddress) throws IOException {
        final Type2Message type2msg = new Type2Message(new Type1Message(Base64.decode(message.substring(5))), this.getChallenge(session, dcAddress),
                null);
     ...
     * @throws IOException
     */
    private NtlmPasswordAuthentication processType3Message(final String message, final HttpSession session, final UniAddress dcAddress)
            throws IOException {
